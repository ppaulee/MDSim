...
std::vector<Particle> &LinkedCells::indexGet(int i) {
    return particles[i];
}
                                ...
                                for (int z_diff = -1; z_diff <= 1; ++z_diff) {
                                    ...
                                    for (auto &p: particles[coordToIndex(c)]) {
                                        ...
                                        if (ArrayUtils::L2Norm(p.getX() - current_particle.getX()) <= cutOffRadius &&
                                            ...
                                            !p.isMarked()) {
                                            // Actual force calculation according to the used algorithm
                                            std::array<double, 3> currentFBefore = current_particle.getF();
...
void LinkedCells::calculateGhostForce(Particle &ghost) {
...
void LinkedCells::handleReflectionBoundary(Particle &p) {
    //for (auto &vec: particles) {
    //if (!vec.empty()) {
